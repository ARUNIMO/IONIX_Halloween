...
pygame.display.set_caption('IONIX_HALLOWEEN')
...
#load images
sun_img = pygame.image.load('moon.png')
bg_img = pygame.image.load('bgi.png')
...
class World():
	def __init__(self, data):
		...
		#load images
		halo_img = pygame.image.load('halo.png')
		...
		for row in data:
			...
			for tile in row:
				if tile == 1:
					img = pygame.transform.scale(halo_img, (tile_size, tile_size))
	...
	if main_menu == True:
		...
		if game_over == 1:
			...
			if level <= max_levels:
				...
				draw_text('YOU WIN! HAPPY HALLOWEEN!! ', font, blue, (screen_width // 2) - 140, screen_height // 2)
				if restart_button.draw():
					level = 1
					#reset level
					world_data = []
